...
Image_Test_PR0 = Y_val32by32[0:2,:,:,:].astype('float32')
center = (int(np.floor(X_val.shape[2] / 2.)), int(np.floor(X_val.shape[3] / 2.)))
Image_Test_PR = Image_Test_PR0[:,:,center[0] - 16:center[0] + 16, center[1] - 16:center[1] + 16] 
Feat_and_Noise   = np.c_[V_Noise_val,V_Feature_val_AE[:,0:size_Feature_R], V_Feature_val_Caption]
...
loss_penality = 0.5*loss_penality1 + 0.5*loss_penality0 # 25 deuxiemes
...
loss_G = temp_G1 + 0.05*(loss_penality)   # 25 deuxieme
...
loss_G_Val = temp_G1_Val + 0.05*(loss_penality)  #0.20
...
#
# Load Model
full_path = '/home/djoumbid/IFT6266/Output/saved_models/generator_epoch_24.pkl'
with open(full_path, 'rb') as f:
    values = pickle.load(f)
lasagne.layers.set_all_param_values(Generator.architecture,values)
full_path = '/home/djoumbid/IFT6266/Output/saved_models/discrminator_epoch_24.pkl'
with open(full_path, 'rb') as f:
    values = pickle.load(f)
...
#            Feature_AE = Feature_AE/(255)
            Feat_and_Noise   = np.c_[Temp_Noise,Feature_AE[:,0:size_Feature_R],Feature_Caption]
...
#            Y_d32by32 = Y_d32by32/(255)
            ...
            center = (int(np.floor(Y_d32by32_V.shape[2] / 2.)), int(np.floor(Y_d32by32_V.shape[3] / 2.)))
            Y_d32by32_N = Y_d32by32[:,:,center[0] - 16:center[0] + 16, center[1] - 16:center[1] + 16] 
            Y_g32by32_N = Y_g32by32[:,:,center[0] - 16:center[0] + 16, center[1] - 16:center[1] + 16] 
            ...
            ##
            C1Q1D = Y_d32by32_N[:,0,12:20,12:20]
            C1Q2D = Y_d32by32_N[:,0,8:24,8:24]
            C1Q3D = Y_d32by32_N[:,0,4:28,4:28]
            C1Q4D = Y_d32by32_N[:,0,0:32,0:32]
            ##
            C2Q1D = Y_d32by32_N[:,1,12:20,12:20]
            C2Q2D = Y_d32by32_N[:,1,8:24,8:24]
            C2Q3D = Y_d32by32_N[:,1,4:28,4:28]
            C2Q4D = Y_d32by32_N[:,1,0:32,0:32]
            ##
            C3Q1D = Y_d32by32_N[:,2,12:20,12:20]
            C3Q2D = Y_d32by32_N[:,2,8:24,8:24]
            C3Q3D = Y_d32by32_N[:,2,4:28,4:28]
            C3Q4D = Y_d32by32_N[:,2,0:32,0:32]
...
#            Y_d32by32 = Y_d32by32/(255)
        ...
        if (batch_idx_val*valid_size >= 38699):
            ...
            Y_g32by32_1 = target[:,:,center[0] - 16:center[0] + 16, center[1] - 16:center[1] + 16] 
            
            Y_g32by32_1 = Y_g32by32_1.astype('float32') 
            Temp = Y_g32by32_1 + 1
            Temp = Temp*(255/2)
...
np.savetxt('Epoch_Penalty_Feature_M35',Epoch_Penalty_Feat)
np.savetxt('Epoch_Penalty_Real_Image_M35',Epoch_Penalty_Real)
####
np.savetxt('Epoch_train_loss_d_M35',Epoch_train_loss_d)  
np.savetxt('Epoch_train_loss_g_M35',Epoch_train_loss_g)  
np.savetxt('Epoch_train_acc_d_M35',Epoch_train_acc_d)  
np.savetxt('Epoch_train_acc_g_M35',Epoch_train_acc_g)  
####
np.savetxt('Epoch_val_loss_d_M35',Epoch_val_loss_D)  
np.savetxt('Epoch_val_loss_g_M35',Epoch_val_loss_G)  
np.savetxt('Epoch_val_acc_d_M35',Epoch_val_acc_D)
np.savetxt('Epoch_Signal_Noise_M35',Epoch_PNR)
